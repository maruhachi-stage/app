...
import {
  hasValidAdminEditKey,
  isAdminEditKeyConfigured,
} from '#presentation/middleware/admin-edit-key.js'
...
export class AdminController {
  ...
  verifyEditKey = async (c: Context<AppEnv>) =>
    c.json(
      successResponse(
        {
          configured: isAdminEditKeyConfigured(),
  ...
  getEditAccess = async (c: Context<AppEnv>) =>
    c.json(successResponse({ available: true }, c.get('requestId')))
}
