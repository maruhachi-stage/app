...
try {
  const [rows] = await connection.query('SELECT GET_LOCK(?, 0) AS acquired', [LOCK_NAME]) as [Array<{ acquired: number }>, unknown]
  ...
  if (!lockAcquired) {
    ...
    const result = spawnSync(process.execPath, [drizzleKit, 'migrate'], { cwd: process.cwd(), env: process.env, stdio: 'inherit' })
