...
export class StaffAuthController {
  ...
  private profile(account: { id?: number; staffAccountId?: number; userId: string; displayName: string; roleId: number }, includeStatus = false) { const role = getStaffRole(account.roleId); return { id: account.id ?? account.staffAccountId, userId: account.userId, displayName: account.displayName, roleId: account.roleId, roleName: role?.name, permissions: role?.permissions ?? [], ...(includeStatus ? {} : {}) } }
