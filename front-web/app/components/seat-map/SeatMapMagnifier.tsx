...
export function SeatMapMagnifier({
    ...
    const dragRef = useRef<{ x: number; y: number; translateX: number; translateY: number } | null>(
        null,
    )
    ...
    const averageSeat = useMemo(() => {
        ...
        return {
            widthPct:
                mapData.seats.reduce((sum, seat) => sum + (seat.widthPct ?? seat.seatWidthPct), 0) /
                mapData.seats.length,
            heightPct:
                mapData.seats.reduce(
                    (sum, seat) => sum + (seat.heightPct ?? seat.seatHeightPct),
                    0,
                ) / mapData.seats.length,
        }
    }, [mapData.seats])
    ...
    const frame = useMemo(() => {
        ...
        const width = compact
            ...
            : clamp(
                  ...
                  360,
                  Math.min(560, containerSize.width - EDGE_PADDING * 2),
              )
        const height = compact
            ? Math.max(0, containerSize.height - EDGE_PADDING * 2)
            : clamp(
                  containerSize.height * 0.72,
                  280,
                  Math.min(480, containerSize.height - EDGE_PADDING * 2),
              )
        ...
        return {
            ...
            top: clampFrame(
                focusY - height / 2,
                EDGE_PADDING,
                containerSize.height - height - EDGE_PADDING,
            ),
            width,
            height,
        }
    ...
    useEffect(() => {
        ...
        setTranslate(
            clampTranslate({ x: rawX, y: rawY }, viewportSize, {
                width: scaledWidth,
                height: scaledHeight,
            }),
        )
