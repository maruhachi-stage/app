...
export class AuthController {
  ...
  getMe = (c: Context<AppEnv>) => {
    ...
    return c.json(
      successResponse(
        session ? { authenticated: true, memberId: session.memberId } : { authenticated: false },
        requestId,
      ),
      200,
    )
  ...
  sendOtp = async (c: Context<AppEnv>) => {
    ...
    try {
      return c.json(
        successResponse(await this.service.sendOtp(input, this.clientIp(c)), requestId),
        200,
      )
    } catch (error) {
  ...
  verifyOtp = async (c: Context<AppEnv>) => {
    ...
    const requestId = c.get('requestId')
    try {
      const result = await this.service.verifyOtp(input, this.clientIp(c))
      createSession(c, { memberId: result.memberId })
      return c.json(successResponse(result, requestId), 200)
    } catch (error) {
      throw this.toAppError(error)
    }
  }
  logout = (c: Context<AppEnv>) => {
    ...
    destroySession(c)
    return c.json(successResponse({ loggedOut: true }, c.get('requestId')), 200)
  }
  private clientIp(c: Context<AppEnv>): string {
    return c.req.header('x-forwarded-for') ?? c.req.header('x-real-ip') ?? 'unknown'
  }
  private toAppError(error: unknown): Error {
    return error instanceof DomainError ? new AppError(error.code, error.message) : (error as Error)
  }
}
