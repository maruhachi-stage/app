  ...
  return (
    <article className="group relative overflow-hidden rounded-lg border border-border bg-card p-3">
      <div className="mb-2 flex items-start justify-between gap-2">
        <p className="line-clamp-2 text-sm font-bold leading-tight text-foreground">{item.title}</p>
        {showNew && (
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        {item.isSoldOut && <span className="text-[10px] font-bold text-destructive">SOLD OUT</span>}
      </div>
    </article>
  )
}
