...
import type { Schedule, ScreeningType } from "~/features/screening/domain/screening"
...
const statusConfig: Record<Status, { label: string; cardClass: string; badgeClass: string; indicator: string }> = {
  available: {
    label: "空席あり",
    cardClass: "border-green-500/40 bg-green-500/10 hover:border-green-500 hover:bg-green-500/15",
    badgeClass: "bg-green-500/20 text-green-700 dark:text-green-400",
    indicator: "◎",
  },
  few: {
    label: "残りわずか",
    cardClass: "border-yellow-500/40 bg-yellow-500/10 hover:border-yellow-500 hover:bg-yellow-500/15",
    badgeClass: "bg-yellow-500/20 text-yellow-700 dark:text-yellow-400",
    indicator: "△",
  },
  full: {
    label: "満席",
    cardClass: "border-red-400/30 bg-red-500/10 opacity-60 cursor-not-allowed pointer-events-none",
        ...
        const content = (
          <>
            <div className="flex items-baseline gap-1 flex-wrap">
              <span className="text-xl font-black text-foreground leading-none tracking-tight">{formatTimeJst(sch.startsAt)}</span>
              <span className="text-xs text-muted-foreground leading-tight" aria-hidden="true">- {formatTimeJst(sch.endsAt)}</span>
            </div>

            <span className="mt-1.5 text-[10px] text-muted-foreground leading-tight">{sch.screenName}</span>

            <div className="mt-2 flex items-center justify-between w-full gap-1">
              <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded ${cfg.badgeClass}`}>
                <span aria-hidden="true">{cfg.indicator}</span>
                {cfg.label}
              </span>
              {!isFull && <span className="text-[10px] text-muted-foreground tabular-nums">残{sch.remainingSeats}</span>}
            </div>
          </>
        )
