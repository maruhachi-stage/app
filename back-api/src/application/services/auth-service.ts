import type { SendOtpRequestDTO, SendOtpResponseDTO, VerifyOtpRequestDTO, VerifyOtpResponseDTO } from '#application/dto/auth.js'
...
export class AuthService {
  constructor(
    ...
    private readonly config: { expiresMin: number; resendSec: number; maxAttempts: number; lockMin: number },
  ) {}
  ...
  async sendOtp(input: SendOtpRequestDTO, ip: string): Promise<SendOtpResponseDTO> {
    ...
    if (await this.tokens.findRecent(memberId, input.type, this.config.resendSec)) {
      throw new DomainError('OTP_RESEND_COOLDOWN', `Wait ${this.config.resendSec} seconds before resending`)
    }
    const code = this.otp.generate()
    await this.tokens.create(memberId, this.otp.hash(code), input.type, new Date(Date.now() + this.config.expiresMin * 60 * 1000))
  ...
  async verifyOtp(input: VerifyOtpRequestDTO, ip: string): Promise<VerifyOtpResponseDTO> {
    ...
    if (token.lockedUntil && new Date(token.lockedUntil) > new Date()) throw new DomainError('OTP_ATTEMPTS_EXCEEDED', `Account locked until ${token.lockedUntil}`)
    if (new Date(token.expiresAt) < new Date()) throw new DomainError('OTP_EXPIRED', 'OTP has expired')
    ...
    if (this.otp.hash(input.code) !== token.tokenHash) {
      ...
      if (attempts >= this.config.maxAttempts) {
        await this.tokens.recordFailure(token.id, attempts, new Date(Date.now() + this.config.lockMin * 60 * 1000))
  ...
  private assertRateLimits(action: 'send' | 'verify', email: string, ip: string, errorCode: 'OTP_RESEND_COOLDOWN' | 'OTP_ATTEMPTS_EXCEEDED'): void {
    if (!this.rateLimiter.check(`otp:${action}:ip:${ip}`, 10, 10 * 60 * 1000)) {
      throw new DomainError(errorCode, action === 'send' ? 'Too many requests from this IP' : 'Too many verification attempts from this IP')
    }
    if (!this.rateLimiter.check(`otp:${action}:email:${email}`, 5, 10 * 60 * 1000)) {
      throw new DomainError(errorCode, action === 'send' ? 'Too many OTP requests for this email' : 'Too many verification attempts for this email')
    }
  }
}
