        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-5">
            ...
            <div className="[perspective:1200px]">
                ...
                >
                    {/* 表面 */}
                    <div
                        className={`absolute inset-0 rounded-app bg-gradient-to-br ${gradient} p-5 sm:p-7 flex flex-col justify-between shadow-2xl overflow-hidden [backface-visibility:hidden]`}
                    >
                            ...
                            <div className="w-10 h-7 rounded bg-gradient-to-br from-yellow-300 to-yellow-500 shadow-inner flex items-center justify-center">
                                <div className="grid grid-cols-2 gap-px w-6">
                                    {[0, 1, 2, 3].map((i) => (
                                        <div
                                            key={i}
                                            className="h-1.5 bg-yellow-700/40 rounded-sm"
                                        />
                                    ))}
                                </div>
                            </div>
                            <span className="text-white/80 font-black text-base tracking-widest">
                                {brand?.label ?? ''}
                            </span>
                        </div>
                        ...
                        <div className="relative flex items-end justify-between">
                            <div>
                                <p className="text-white/40 text-[8px] uppercase tracking-widest mb-0.5">
                                    Valid Thru
                                </p>
                                <p className="text-white font-mono font-bold text-sm tracking-widest">
                                    {expiry || 'MM/YY'}
                                </p>
                            </div>
                            {brand?.id === 'mastercard' && (
                            ...
                            {brand?.id === 'visa' && (
                                <span className="text-white/70 font-black text-xl italic tracking-tight">
                                    VISA
                                </span>
                            )}
                            {brand && !['mastercard', 'visa'].includes(brand.id) && (
                                <span className="text-white/60 font-black text-sm tracking-widest">
                                    {brand.label}
                                </span>
                            )}
                        </div>
                    ...
                    {/* 裏面 */}
                    <div
                        className={`absolute inset-0 rounded-app bg-gradient-to-br ${gradient} flex flex-col gap-6 shadow-2xl overflow-hidden [backface-visibility:hidden] [transform:rotateY(180deg)]`}
                    >
                        ...
                        <div className="px-6 flex flex-col gap-3">
                            ...
                            <p className="text-white/30 text-[9px] text-right">
                                カード裏面3桁のコード
                            </p>
                        </div>
                    </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] text-muted-foreground/50 font-bold uppercase tracking-wider mr-1">
                    対応
                </span>
                {CARD_BRANDS.map((b) => (
                    <span
            ...
            <div className="rounded-app border border-border bg-card p-6 flex flex-col gap-5">
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-border">
                    ...
                    <p className="text-[10px] text-muted-foreground/40 leading-tight">
                        SSL/TLS
                        暗号化通信で保護されています。※デモのためカード情報は送信されません。
                    </p>
                </div>
            </div>
