...
try {
  ...
  const userId = (await rl.question('ユーザーID: ')).trim()
  const displayName = (await rl.question('表示名: ')).trim()
  const email = (await rl.question('OTP送信先メールアドレス: ')).trim()
  const password = await rl.question('パスワード（12文字以上）: ')
  const roleId = Number(await rl.question(`ロールID (${Object.values(STAFF_ROLES).map((role) => `${role.id}:${role.name}`).join(', ')}): `))
