...
export default function ScreeningDetailPage() {
    ...
    const { data, loading, error, days, selectedDate, setDate, selectedType } = useSchedules({
        idParamName: 'movieId',
    })
    ...
    useEffect(() => {
        ...
        const endpoint =
            selectedType === 'stage' || selectedType === 'event'
                ? `/stages?type=${selectedType}`
                : '/movies'
    ...
    const relatedProducts = products
        .filter(
            (product) => !item?.title || !product.movieTitle || product.movieTitle === item.title,
        )
        .slice(0, 8)
