...
export function useScheduleSelection() {
    ...
    const selectedScheduleId = searchParams.get('scheduleId')
        ? Number(searchParams.get('scheduleId'))
        : null
    ...
    useEffect(() => {
        if (!selectedDate && selectedScheduleId) {
            apiFetch<ScheduleInfo>(`/schedules/${selectedScheduleId}`).then((sch) => {
                const date = new Date(sch.startsAt).toLocaleDateString('sv', {
                    timeZone: 'Asia/Tokyo',
                })
                setSearchParams(
                    (prev) => {
    ...
    useEffect(() => {
        ...
        const endpoint =
            selectedType === 'stage' || selectedType === 'event'
                ? `/stages/${movieId}/schedules?date=${selectedDate}`
                : `/movies/${movieId}/schedules?date=${selectedDate}`
