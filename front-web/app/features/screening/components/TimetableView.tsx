...
function ScreeningLabel({
    ...
    if (compact) {
        return (
            <div
                className="flex h-full items-end justify-center bg-muted/40 p-1.5"
...
export default function TimetableView({ screenings, selectedDate }: Props) {
    ...
    return (
        <>
            <TimetableTable
                screenings={screeningsWithSchedules}
            ...
            <div className="overflow-x-auto" onScroll={handleBodyScroll}>
                <div style={tableWidthStyle}>
                    {screenings.map((screening) => {
                        const { positionedSchedules, laneCount } = layoutSchedules(
                            screening.schedules!,
                        )
                            ...
                            >
                                ...
                                <div className="relative flex-1 py-2" style={{ height: rowHeight }}>
                                    ...
                                    {positionedSchedules.map(({ schedule, lane }) => {
                                        ...
                                        const endMin = toJstMinutes(schedule.endsAt)
                                        const left =
                                            ((startMin - HOUR_START * 60) / TOTAL_MINUTES) * 100
                                        ...
                                        const laneTopOffset =
                                            laneInset -
                                            laneRatio *
                                                (laneInset * 2 + laneGap * (laneCount - 1)) +
                                            lane * laneGap
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <p
                                                    className={`${compact ? 'text-[9px]' : 'text-[10px]'} whitespace-nowrap leading-tight opacity-80`}
                                                >
                                                    {formatTimeJst(schedule.startsAt)}〜
                                                    {formatTimeJst(schedule.endsAt)}
                                                </p>
                                            </Link>
                                        )
                                    })}
