    ...
    return (
        ...
        >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6">
                <span className="inline-block bg-primary text-primary-foreground text-[11px] font-bold tracking-widest px-2.5 py-1 rounded-sm mb-3">
                    上映中
                </span>
                <h2 className="text-2xl font-bold leading-snug text-white drop-shadow-lg mb-2">
                    {movie.title}
                </h2>
                <p className="text-white/70 text-xs mb-4 line-clamp-2">{movie.description}</p>
                <span className="inline-flex items-center gap-1.5 bg-primary/90 hover:bg-primary text-primary-foreground text-sm font-semibold px-5 py-2.5 rounded transition-colors">
                    ▶ 予約する
                </span>
            </div>
        </Link>
...
function HeroSub({ movie }: { movie: Screening }) {
    ...
    return (
        <Link
            to={movieDetailPath(movie.id)}
            className="relative block overflow-hidden rounded-lg group aspect-square sm:aspect-auto"
        >
            {imgUrl ? (
                <>
                    <img
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5">
                <span className="inline-block text-[10px] font-bold tracking-widest px-2 py-0.5 rounded-sm mb-2 bg-primary/80 text-primary-foreground">
                    上映中
                </span>
                <h3 className="text-base font-bold leading-snug text-white drop-shadow-lg line-clamp-2">
                    {movie.title}
                </h3>
    ...
    return (
        <section>
            <div className="flex items-center gap-3 mb-6">
                ...
                <Link
                    to={moviesListPath({ status: 'now_showing' })}
