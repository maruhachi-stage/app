import type { CreateMemberRequestDTO, MemberDTO, ReservationListResponseDTO } from '#application/dto/members.js'
...
export class MemberService {
  constructor(private readonly members: MemberRepository, private readonly reservations: MemberReservationRepository) {}
  ...
  async create(input: CreateMemberRequestDTO): Promise<{ member: MemberDTO; created: boolean }> {
    ...
    if (existing) return { member: { id: existing.id, email: existing.email, name }, created: false }
