...
export function useScreenings(options?: UseScreeningsOptions) {
    ...
    const selectedType =
        options?.type ??
        options?.category ??
        ((searchParams.get('type') ?? 'all') as ScreeningType | 'all')
    ...
    useEffect(() => {
        ...
        if (selectedType === 'all') {
            fetchPromise = Promise.all([
                apiFetch<{ items: Screening[] }>(`/movies${qs ? `?${qs}` : ''}`).then((d) =>
                    d.items.map((item) => ({ ...item, type: 'movie' as const })),
                ),
                apiFetch<{ items: Screening[] }>(`/stages${qs ? `?${qs}` : ''}`).then((d) =>
                    d.items.map((item) => ({ ...item, type: item.type ?? ('stage' as const) })),
                ),
            ]).then(([movies, stages]) => ({
                items: [...movies, ...stages],
            }))
            ...
            const endpoint =
                selectedType === 'stage' || selectedType === 'event' ? '/stages' : '/movies'
            const typeQs =
                selectedType === 'stage' || selectedType === 'event' ? `type=${selectedType}` : ''
            const fullQs = [qs, typeQs].filter(Boolean).join('&')
            fetchPromise = apiFetch<{ items: Screening[] }>(
                `${endpoint}${fullQs ? `?${fullQs}` : ''}`,
            ).then((d) => ({
