...
function buildDetailUrl(movie: Movie, selectedDate: string): string {
  const type = movie.type ?? "movie"
  const qs = new URLSearchParams({ type })
  if (selectedDate) qs.set("date", selectedDate)
  return `/screenings/${movie.id}?${qs.toString()}`
}

export function MovieGridCard({ movie, selectedDate }: Props) {
  const today = todayJst()
  const type = movie.type ?? "movie"
  const detailUrl = buildDetailUrl(movie, selectedDate)
  const bookingUrl = `/reservations/booking/${movie.id}?date=${today}&type=${type}`
    ...
    <div className="group flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5 px-0.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-[10px] font-medium text-muted-foreground">{movie.durationMin}分</span>
        </div>
      </div>
    </div>
  )
