...
export function LoginPage() {
  ...
  return <main className="grid min-h-dvh place-items-center bg-admin-canvas p-4">
    <section className="w-full max-w-md rounded-xl border border-admin-border bg-admin-surface p-6 shadow-sm">
      <p className="text-sm font-black tracking-[0.18em] text-admin-accent">HAL CINEMA</p>
      <h1 className="mt-2 text-2xl font-black">スタッフログイン</h1>
      <p className="mt-2 text-sm text-admin-muted-foreground">{step === "password" ? "ユーザーIDとパスワードを入力してください。" : "メールで受け取った6桁の認証コードを入力してください。"}</p>
      {step === "password" ? <form className="mt-6 grid gap-4" onSubmit={submitPassword}>
        ...
        <button type="button" className="text-sm font-bold text-admin-muted-foreground" onClick={() => { setStep("password"); setCode(""); setError("") }}>別のユーザーIDでログイン</button>
      </form>}
      {message && <p className="mt-4 text-sm font-bold text-admin-success">{message}</p>}
      {error && <p className="mt-4 text-sm font-bold text-admin-danger">{error}</p>}
    </section>
  </main>
}
