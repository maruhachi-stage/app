// Presentation: 予約フローのContext Provider
import { createContext, useContext, useState, useCallback, type ReactNode } from 'react'
import type {
    ReservationFlowState,
    FlowStep,
    GuardResult,
} from '~/features/reservation/domain/flow'
...
import { canProceed } from '~/features/reservation/domain/flow-guards'
...
export function ReservationFlowProvider({ children }: { children: ReactNode }) {
    ...
    const setCustomer = useCallback((customer: { email: string }) => update({ customer }), [update])

    const setTickets = useCallback(
        (seats: SelectedSeat[], counts: TicketCounts, totalPrice: number) =>
            update({ selectedSeats: seats, ticketCounts: counts, totalPrice }),
        [update],
    )
    ...
    const completeFlow = useCallback((result: Parameters<ContextValue['completeFlow']>[0]) => {
